...
content_cid=$(/usr/bin/time -f 'upload-content: %e\n' "${SCRIPTS_DIR}/deploy/upload_dir_to_cluster.sh" "$1")
printf 'Added content to cluster with CID %s\n' "${content_cid}"

service_cid=$(/usr/bin/time -f 'upload-service: %e\n' "${SCRIPTS_DIR}/deploy/upload_file_to_cluster.sh" "$2/service.json")
printf 'Added service.json to cluster with CID %s\n' "${service_cid}"

printf 'Providing service CID %s\n' "${service_cid}"
/usr/bin/time -f 'service-provide: %e\n' "${SCRIPTS_DIR}/deploy/provide.sh" "${service_cid}" 'false'
printf 'Providing content CID %s\n' "${content_cid}"
/usr/bin/time -f 'content-provide: %e\n' "${SCRIPTS_DIR}/deploy/provide.sh" "${content_cid}" 'true'
